<template>
    <draggable :list="slices" group="gusti" :class="'flex flex-row flex-wrap shadow bg-gray-300 border-4 border-black border-dashed ' + dragHeight" @change="change">
        <template v-for="(slice,index) in slices">
            <div :key="'slice_' + index" class="w-full cursor-move items-center relative flex flex-row justify-between p-2">
...
export default {
    ...
    mounted(){
        ...
        this.slices = this.$store.getters.cart.items.tranci
